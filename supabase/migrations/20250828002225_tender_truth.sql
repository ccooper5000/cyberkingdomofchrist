...
-- (No public insert/update policy; use service role/webhook)
...
using (
  visibility = 'public'
  -- group/circle scoping can be layered later via additional policies or views
);
...
-- server role (edge function) should update status; no public update/delete policy.

-- ---- moderation_flags (admin-only; create a role later if needed)
...
-- ---- audit_logs (admin-only read/write via server role)
...
-- ---- stripe_events (service/webhook only)
